...
### 🧭 성장 흐름
**아두이노 임베디드**(졸업프로젝트 · 모터 차동 제어) → **ROS2 자율주행 대회**(ERP42 · 팀장) → **협동로봇·AMR 프로젝트 계획~결과보고**(ROKEY 4개) → **예외처리·논문/오픈소스 기반 설계·실기 테스트·문서화**(VLA Pick & Place)
...
<a id="vla"></a>
### 🦾 VLA 기반 자연어 Pick & Place — 구동 중 장애물 정지 후 재계획
**LLM 판단과 상태머신 안전의 제어 권한을 하나로 모으고, 구동 중 장애물 앞에서 멈춘 뒤 새 경로로 이어 가게 했습니다**
...
- **개요:** "사과 바구니에 담아줘" 같은 자연어 지시를 LLM이 해석하고, 협동로봇이 물체를 인식해 집어 옮기는 시스템
- **위치:** ERP42 → 부트캠프 ROS2 프로젝트 3개를 거쳐, 판단·인식·모션·안전을 하나로 통합한 마지막 프로젝트
- **문제 ① 제어 주체가 둘:** LLM 판단 계층과 안전 계층(pick FSM — 단계별 상태를 나눠 중단 지점부터 재개하는 상태머신)이 각자 로봇에 명령 가능
  - → 제어 권한을 안전 계층 하나로 모으고, 판단 계층은 "무엇을 어디로"만 넘기게 통합
- **문제 ② 에러 없는 실패:** 장애물 인식이 멈춰도 경로 계획은 "장애물 없는 세상" 기준으로 성공
  - → 원인 후보 3개(서비스 미기동·설정 오류·반영 지연)로 나누고, 로봇을 움직이지 않고 계획만 실행해 재현
  - → 팀 모션 코드 위에 **인식 서비스가 없으면 이동을 거부하는 게이트** + 반영된 장애물 수 기록, 설정 오류는 사전 점검 모드로 검출
- **문제 ③ 구동 중 장애물:** 이동 중 새 장애물이 경로를 막으면 멈추고 새 경로로 이어 가야 함
  - → FSM에 중복 구현하지 않고 MoveIt `move_group` 내장 재계획에 연결, FSM은 재계획 실패 시 재시도만 담당
  - → OMPL 경로는 동작, cuMotion(GPU 플래너) 경로는 장애물을 밀고 가는 것을 실기 확인 → "계획 시점에만 장애물을 읽는다"는 가설로 반응형 회피(RMPflow) 검토
- **결과·한계:** OMPL 경로의 구동 중 정지→재계획, 출발 전 장애물 우회를 실기 확인. cuMotion 실행 중 회피·전체 pick-to-place 실물 검증은 남은 과제
...
**인지 일부가 빠져도 버티는 측위·경로추종 기반을 팀원들과 만들고, 팀장으로 실차 대회를 완주했습니다**
...
- **결과:** 제작비 약 51만 원(상용 대비 약 1/10), 교내 우수캡스톤·설계경진대회 우수상. 폐회로 제어 전환은 다음 과제
...
1. **조용한 실패를 쪼개 잡습니다** — "성공처럼 보이는" 문제를 원인 후보로 나눠 재현합니다(VLA 인지 게이트, AMR 신호 흔들림, 커피 파지 판정).
2. **판단과 안전을 분리해 통합합니다** — 제어 권한을 한 곳으로 모으고, 명령이 없거나 낡으면 멈추게 설계합니다(VLA 계층 통합, ERP42 판단 노드 prototype).
3. **실기로 확인하고 기록합니다** — 실측 제약과 실패를 다음 실험·인수인계의 근거로 남깁니다(VLA 실측 제약 문서, ERP42 실차 테스트·인수인계).
