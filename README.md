...
전공 수업에서 아두이노로 모터를 제어해 라인트레이서 제작으로 시작해, 무인모빌리티 자율주행 대회 팀장을 거쳐 ROS2 기반 협동로봇·AMR 프로젝트에서 상태관리, 안전 게이트, 인지-모션 통합을 맡았습니다.
각 카드에는 프로젝트에서 구현한 기술을 모두 적고, 그중 역할 분담상 제가 맡은 부분을 강조 표시했습니다.

📫 gwanhuig01@gmail.com
...
| 🛠️ 스킬 | `ROS·ROS2` `Python` `C++` `Ubuntu` `Solidworks` `Arduino` `SolidWorks` 'OpenCV'  |
...
> ### 🔖 대표 프로젝트
> 🦾 **VLA 기반 자연어 Pick & Place - 협동로봇 구동 중 장애물 감지 및 경로 수정 로직 구현** — 판단·안전 계층 통합 + 대조군 A/B로 재계획 로직 검증 · 부트캠프 앞선 3개 ROS2 프로젝트 경험을 모은 마지막 작업 · `FSM(Finity State Machine, 로봇의 상태를 구분하고 단계 모니터링 및 중단 시 반복 가능하도록 설계)` `안전 게이트` `LLM function calling`
...
Arduino Uno · 3인 팀 · 충남대 기계공학부 캡스톤디자인 (24.08~25.08)
...
- **프로젝트 기술:**
  ...
  - **발사장치:** 공기저항을 고려한 2차원 포물선 운동방정식으로 필요 힘을 산정해 스프링 선정, 컴파운드 석궁 방식 트리거·금속 몸체 제작(밀링, 용접 등)
